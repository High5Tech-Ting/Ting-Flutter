...
class StaffManagementTab extends StatefulWidget {
  const StaffManagementTab({Key? key}) : super(key: key);
...
class _StaffManagementTabState extends State<StaffManagementTab> {
  ...
  void _performSearch(String searchTerm) async {
    ...
    final results = await UserSearchService.searchStaffByStaffId(searchTerm);
      ...
      return const Center(
        child: Column(
          ...
          children: [
            ...
            SizedBox(height: 8),
            Text(
              'Try searching with a different Staff ID',
              style: TextStyle(
                color: Colors.grey,
              ),
            ),
  ...
  Widget _buildAllStaff() {
    return StreamBuilder<QuerySnapshot>(
      stream: UserSearchService.getAllUsersOfType('staff'),
